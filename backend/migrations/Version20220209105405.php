 ...
 */
final class Version20220209105405 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, password, name, email, organization, roles, enabled, confirmation_token, is_partner FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, name VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, organization VARCHAR(255) DEFAULT NULL COLLATE BINARY, roles CLOB DEFAULT NULL --(DC2Type:simple_array)
        , enabled BOOLEAN NOT NULL, confirmation_token VARCHAR(40) DEFAULT NULL COLLATE BINARY, fully_registered BOOLEAN DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, username, password, name, email, organization, roles, enabled, confirmation_token, fully_registered) SELECT id, username, password, name, email, organization, roles, enabled, confirmation_token, is_partner FROM __temp__user');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE INDEX IDX_9474526CF675F31B ON comment (author_id)');
        $this->addSql('CREATE INDEX IDX_9474526CA77FBEAF ON comment (blog_post_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, password, name, email, roles, organization, enabled, confirmation_token, fully_registered FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, roles CLOB DEFAULT \'NULL --(DC2Type:simple_array)\' COLLATE BINARY --(DC2Type:simple_array)
        , organization VARCHAR(255) DEFAULT NULL, enabled BOOLEAN NOT NULL, confirmation_token VARCHAR(40) DEFAULT NULL, is_partner BOOLEAN DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, username, password, name, email, roles, organization, enabled, confirmation_token, is_partner) SELECT id, username, password, name, email, roles, organization, enabled, confirmation_token, fully_registered FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
